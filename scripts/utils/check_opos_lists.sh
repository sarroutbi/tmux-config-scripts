...
function retrieve_files()
{
  ...
  get_file "${LINK}" "${GLOBAL_LINK}" "Puntuaciones de últimos participantes citados para cubrir puestos de carácter voluntario" ${TMP_LAST_VOLUNTEER_CALLED}
...
function retrieve_pri_files()
{
  ...
  get_file "${PRIMARY_LINK}" "${GLOBAL_LINK}" "Puntuaciones de últimos participantes citados para cubrir puestos de carácter voluntario" ${TMP_PRI_LAST_VOLUNTEER_CALLED}
